        ...
        //    }
            ...
            if(cboProducto.SelectedIndex != 0)
            {
                if (!existe(cboProducto.Text))
                {
                    ...
                    dt.Cantidad = Convert.ToInt32(nudCantidad.Value);
                    receta.AgregarDetalle(dt);
                    dgvDetalles.Rows.Add(new object[] { dt.Ingrediente.IdIngrediente, dt.Ingrediente.Nombre, dt.Cantidad });
                    lblTotalIngredientes.Text = dgvDetalles.Rows.Count.ToString();
                }
                
            }
        }
        public bool existe(string selecteditem)
        {
            bool aux = false;
            foreach (DataGridViewRow fila in dgvDetalles.Rows)
            {
                if (fila.Cells["ingrediente"].Value.ToString().Equals(selecteditem))
                {
                    aux = true;
                    break;
                }
